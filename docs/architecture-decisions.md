...
| ADR-004 | Separate scientific search from social ranking | Product owner + scientific lead | Accepted | Objective metadata, quality, provenance, and validated similarity determine scientific discovery; reactions and trends belong only in community views. | Never |
...
| ADR-007 | Google and ORCID identity | Product owner + technical lead | Provisional | Use Google as the first login method and ORCID as a linked verified research identity. Do not add ORCID sign-in until account-linking and recovery semantics are defined. | Before Stage 2 implementation |
