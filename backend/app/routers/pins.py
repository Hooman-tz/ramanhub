...
owner-only, and there is no way to pin something you do not own or have not
published: pinning someone else's spectrum, or a private draft, to your
profile would misrepresent authorship or leak unpublished work.
"""
...
def add_pin(
    ...
    if body.kind == "spectrum":
        ...
        is_published = bool(target and target.state == SpectrumState.published)
    else:
        target = db.get(Finding, body.id)
        owner_id = target.owner_id if target else None
        is_published = bool(target and target.state == FindingState.published)
    if target is None or owner_id != user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")
    if not is_published:
        # The item is yours but still a draft. Pinning it would surface
        # unpublished work on a page anyone can load.
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="You can only pin published work.",
        )

    existing = (
        db.scalar(select(func.count()).select_from(Pin).where(Pin.user_id == user.id)) or 0
    )
    ...
    try:
        ...
        # toggle in the UI and a double submit should not 500. Exiting the
        # `begin_nested()` block has already unwound the SAVEPOINT; a full
        # rollback here would discard everything else the session had done.
...
def remove_pin(
    ...
    if pin is not None:
        ...
        db.delete(pin)
        db.flush()
        # Renormalize: pins after the hole slide down one, so positions stay
        # a dense 0..n-1 run and the next add lands at the right index.
        for other in db.scalars(
            select(Pin)
            .where(Pin.user_id == user.id, Pin.position > removed_position)
            .order_by(Pin.position)
        ).all():
            other.position -= 1
        db.commit()
    return _pins_for(user.id, db)
