...
Mounted with no prefix. The thread model is described in
`app.models.finding`; this router is the write path for it.

Two rules run through everything here:

1. **Publishing a Finding requires its spectra to be published.** A public
   write-up whose figures point at private data renders as a wall of 404s,
   and worse, its abstract can describe data nobody can check. Enforced in
   `_assert_publishable`.
...
def _assert_publishable(finding: Finding, db: Session) -> None:
    ...
    Otherwise the write-up renders as broken figures for every reader but
    the author, and its claims cite data nobody can inspect — which defeats
    ...
    if not members:
        raise HTTPException(
            ...
            detail="Add at least one spectrum before publishing.",
        )
    private = [
...
def list_my_findings(
    ...
    view. Public discovery goes through `/feed` and `/search`."""
    findings = db.execute(
        select(Finding)
