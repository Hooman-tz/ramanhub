...
def suggest_handle(email: str, display_name: str | None = None) -> str:
    """Build a plausible base handle from what Google OAuth gives us.
...
def assign_handle(db, email: str, display_name: str | None = None) -> str:
    ...
    on `users.handle` is the real guarantee — the loser gets an
    ...
    def exists(candidate: str) -> bool:
        return db.query(User.id).filter(User.handle == candidate).first() is not None
