...
costs; one number that blends them tells you nothing about which happened.
The three per-day series are returned separately for the UI to stack.

**UTC buckets.** Days are bucketed in UTC to match the calendar the streak
logic and the `days` window are built from.
"""
...
def _daily_counts(db: Session, column, filters) -> dict[date, int]:
    ...
    # already the next day in UTC, so today's uploads get filed under
    # yesterday and today's square renders empty — which also breaks the
    # current streak. `timezone('UTC', col)` pins the bucket to the same
    # clock the calendar uses.
